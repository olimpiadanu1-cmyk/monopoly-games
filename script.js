...
function startLeaderboardAutoUpdate() {
    ...
    setInterval(async () => {
        if (currentUser && document.getElementById('monopoly-page') && !document.getElementById('monopoly-page').classList.contains('hidden')) {
            // Перезагружаем данные пользователей с сервера
...
async function loadGameState() {
    ...
    try {
        const gameStates = await monopolyAPI.loadData('game_states') || {};
        const gameState = gameStates[currentUser.id];
        ...
        console.warn('⚠️ Ошибка загрузки игрового состояния:', error.message);
    }
}
...
async function showMonopolyGame() {
    if (!currentUser) {
        showAuthPage();
        return;
    }
...
async function loginUser() {
    ...
    // Проверяем, есть ли сохраненное состояние на сервере
    const gameStates = await monopolyAPI.loadData('game_states') || {};
    const savedState = gameStates[currentUser.id];
    if (!savedState) {
...
async function createLeaderboard() {
    ...
    // Загружаем игровые состояния с сервера
    const gameStates = await monopolyAPI.loadData('game_states') || {};
    
    // Создаем массив игроков с их позициями
    let players = registeredUsers.map(user => {
        // Загружаем игровое состояние каждого пользователя с сервера
        const gameState = gameStates[user.id];
...
async function updatePlayerStatus() {
    ...
    // Загружаем актуальный лидерборд с сервера
    const players = await monopolyAPI.loadData('leaderboard') || [];
...
async function submitTaskForReview() {
    ...
    // Сохраняем на сервер
    await monopolyAPI.saveData('task_submissions', taskSubmissions);
    
    // Обновляем статус задания для пользователя в игровом состоянии
    const gameStates = await monopolyAPI.loadData('game_states') || {};
    if (!gameStates[currentUser.id]) {
        gameStates[currentUser.id] = {
            position: currentPosition,
    ...
    if (!gameStates[currentUser.id].taskStatuses) {
        gameStates[currentUser.id].taskStatuses = {};
    }
    
    gameStates[currentUser.id].taskStatuses[currentPosition] = {
        status: 'pending',
...
async function loadApplications() {
    // Перезагружаем заявки с сервера для актуальных данных
...
async function loadUsers() {
    // Перезагружаем пользователей с сервера для актуальных данных
    users = await monopolyAPI.loadData('users') || [];
...
async function loadShopItems() {
    ...
    shopItems = await monopolyAPI.loadData('shop_items') || [];
...
async function resetAllPlayersToStart() {
    if (!currentUser || !currentUser.isAdmin) {
    ...
    const allUsers = await monopolyAPI.loadData('users') || [];
...
async function autoResetAllPlayers() {
    if (!currentUser || !currentUser.isAdmin) {
...
async function approveTask(submissionId) {
    ...
    // Сохраняем обновленные отправки на сервер
    await monopolyAPI.saveData('task_submissions', taskSubmissions);
    ...
    if (!gameStates[submission.userId]) {
        gameStates[submission.userId] = {
            position: 1,
            moves: 30,
            points: 0,
            completed: false,
    ...
    if (!gameStates[submission.userId].taskStatuses) {
        gameStates[submission.userId].taskStatuses = {};
    }
    
    gameStates[submission.userId].taskStatuses[submission.taskNumber] = {
        status: 'approved',
    ...
    gameStates[submission.userId].moves = (gameStates[submission.userId].moves || 30) + 1;
    
    // Сохраняем обновленные игровые состояния на сервер
    await monopolyAPI.saveData('game_states', gameStates);
...
async function giveMoves() {
    ...
    // Перезагружаем пользователей с сервера
    users = await monopolyAPI.loadData('users') || [];
    ...
    // Загружаем игровые состояния с сервера
    const gameStates = await monopolyAPI.loadData('game_states') || {};
    let gameState = gameStates[user.id];
...
async function addRewardHistory(type, nickname, amount) {
    ...
    rewardHistory.unshift(record); // Добавляем в начало массива
    
    // Ограничиваем историю 50 записями
    if (rewardHistory.length > 50) {
        rewardHistory.splice(50);
    }
    
    // Сохраняем на сервер
    await monopolyAPI.saveData('reward_history', rewardHistory);
...
async function loadPurchaseRequests() {
    // Перезагружаем заявки на товары с сервера для актуальных данных
    purchaseRequests = await monopolyAPI.loadData('purchase_requests') || [];
...
async function loadAllDataFromServer() {
    ...
    try {
        ...
        if (allData) {
            ...
            taskSubmissions = allData.task_submissions || [];
            rewardHistory = allData.reward_history || [];
            
            // Загружаем дополнительные данные
            if (allData.task_submissions) {
                // task_submissions уже загружены выше
            }
            if (allData.reward_history) {
                // reward_history уже загружены выше
            }
            if (allData.leaderboard) {
                // leaderboard уже загружены выше
            }
